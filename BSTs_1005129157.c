...
// //HELPER FUNCTION
void find_freq_node(BST_Node *root, double freq, double new_freq){
	
	if(root == NULL){
		return;
	}

	if(fabs(freq - (root -> freq)) < 0.000000000001){	
		root -> freq = new_freq;
	}
	
	find_freq_node(root -> left, freq, new_freq);
	
	find_freq_node(root -> right, freq, new_freq);
	...
	double freq;
	 ...
	 for(int i = 0; i < 100; i ++){
		...
		for(int x = 0; x < 5; x ++){
			...
			if(note_names[i][x] != note_dst[x]){
				break;
			}
			
			if (note_dst[x] == '\0'){
				new_freq = note_freq[i];
			}
	 ...
	 if(freq != 0 && new_freq != 0){
		find_freq_node(root,freq,new_freq);
	 }
...
BST_Node* create_harmonized_notes(BST_Node *root, int semitones, double time_shift, BST_Node* og_root){
	
	if(root == NULL){
        return NULL;
    }
	
	create_harmonized_notes(root -> left, semitones, time_shift, og_root);
	 
	create_harmonized_notes(root -> right, semitones, time_shift, og_root);
	
	int temp_freq = -1;
	
	for(int i = 0; i < 100; i ++){
		if(fabs(note_freq[i] - (root -> freq)) < 0.000000000001){
			temp_freq = i;
		} 
	}

	if(temp_freq != -1 && temp_freq + semitones <= 99 && temp_freq + semitones >= 0 && root -> index + time_shift > 0 && root -> index + time_shift <= 1){	
		BST_Node* p = NULL;
		p = newBST_Node(note_freq[temp_freq + semitones], root -> bar , root -> index + time_shift);
		BST_insert(og_root,p);
	}
...
BST_Node *BST_harmonize(BST_Node *root, int semitones, double time_shift)
{
	...
	return create_harmonized_notes(root, semitones, time_shift, root);
